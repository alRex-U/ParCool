...
public abstract class Action {
    ...
    public void startExplicitly() {
        ...
        if (clientSide) {
            PacketHelper.sendToServer(
                    ActionStateSetPacket.HANDLER.id(), packet, ActionStateSetPacket.HANDLER::encode
            );
        } else {
    ...
    protected final boolean isPossible() {
        ...
        if ((option.neededPose() != null && option.neededPose() != player.getPose())
                || (!option.availableInFluid() && player.isInWater())
                || (!option.availableNotInFluid() && !player.isInWater())
    ...
    public boolean isReadyToStart() {
        ...
        if (ParCoolActionArchEvent.TryToStart.EVENT.invoker().onTryToStart(parkourability.player(), this).isFalse())
            return false;
