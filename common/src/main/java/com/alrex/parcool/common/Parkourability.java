...
public class Parkourability {
    ...
    public boolean permit(ActionEntry<?> actionEntry) {
        ...
        return config.get(actionEntry).permit().get() && (!config.system().enableSkillTree().get() || capabilities.can(actionEntry)) && enabledActionStates.can(actionEntry);
    }

    /// Request the action start.
    /// All requests are cleared on tick finishing, so this must be called before the action is ticked
    public <Context, RequestableAction extends Action & IRequestable<Context>> void request(ActionEntry<RequestableAction> actionEntry, Context context) {
