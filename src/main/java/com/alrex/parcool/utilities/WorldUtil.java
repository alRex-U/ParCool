...
public class WorldUtil {
	...
	public static Vector3d getWall(LivingEntity entity) {
		...
		if (!entity.world.hasNoCollisions(baseBox.expand(distance, 0, 0))) {
			wallX++;
			wallNumX++;
		}
		if (!entity.world.hasNoCollisions(baseBox.expand(-distance, 0, 0))) {
		...
		if (!entity.world.hasNoCollisions(baseBox.expand(0, 0, distance))) {
			wallZ++;
			wallNumZ++;
		}
		if (!entity.world.hasNoCollisions(baseBox.expand(0, 0, -distance))) {
	...
	public static Vector3d getStep(LivingEntity entity) {
		...
		if (!world.hasNoCollisions(baseBoxSide.expand(distance, 0, 0)) && world.hasNoCollisions(baseBoxTop.expand(distance, 0, 0))) {
			stepX++;
		}
		if (!world.hasNoCollisions(baseBoxSide.expand(-distance, 0, 0)) && world.hasNoCollisions(baseBoxTop.expand(-distance, 0, 0))) {
			stepX--;
		}
		if (!world.hasNoCollisions(baseBoxSide.expand(0, 0, distance)) && world.hasNoCollisions(baseBoxTop.expand(0, 0, distance))) {
			stepZ++;
		}
		if (!world.hasNoCollisions(baseBoxSide.expand(0, 0, -distance)) && world.hasNoCollisions(baseBoxTop.expand(0, 0, -distance))) {
			stepZ--;
