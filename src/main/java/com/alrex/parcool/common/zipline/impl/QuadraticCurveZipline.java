...
public class QuadraticCurveZipline extends Zipline {
    public QuadraticCurveZipline(Vector3f point1, Vector3f point2) {
        super(point1, point2);
    }
    ...
    public Vector3d getMidPoint(float t) {
        ...
        return getMidPointOffsetFromStart(t).add(start.x(), start.y(), start.z());
    }

    @Override
    public Vector3d getMidPointOffsetFromStart(float t) {
        float x = getOffsetToEndFromStart().x() * t;
        float z = getOffsetToEndFromStart().z() * t;
        float y = getOffsetToEndFromStart().y() * t * t;
        return new Vector3d(x, y, z);
    }

    @Override
    public float getSlope(float t) {
        return 2 * getOffsetToEndFromStart().y() * MathHelper.fastInvSqrt(getOffsetToEndFromStart().x() * getOffsetToEndFromStart().x() + getOffsetToEndFromStart().z() * getOffsetToEndFromStart().z());
    }

    @Override
    public float getParameter(Vector3d position) {
        float offsetX = getOffsetToEndFromStart().x();
        float offsetZ = getOffsetToEndFromStart().z();
    ...
    public double getMovedPositionByParameterApproximately(float currentT, float movement) {
        ...
        double xzLength = Math.sqrt(getOffsetToEndFromStart().x() * getOffsetToEndFromStart().x() + getOffsetToEndFromStart().z() * getOffsetToEndFromStart().z());
